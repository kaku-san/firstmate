...
- **Explicit delivery modes** - every ship task is dispatched with an explicit `no-mistakes`, `direct-PR`, or `local-only` mode and yolo autonomy posture, resolved at intake from your instruction or the project's registered standing posture.
- **Optional secondmates** - opt in to persistent second mates that run from isolated firstmate homes with their own `FM_HOME`, state, projects, and session lock, either locally or as a whole home on an SSH-reachable host; local routes can supervise project clones or a project-less firstmate-repo domain, while remote routes keep the entire home on that host with guarded updates and recovery that never turns an unavailable remote route into a local replacement.
- **Event-driven, zero-token supervision** - a bash watcher sleeps on the fleet and wakes the first mate only when something needs you; verified primary harnesses also get a turn-end backstop that blocks or follows up on a blind stop whenever supervision is needed and not live.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
  ...
  treehouse worktree, Orca worktree, or isolated secondmate home
     │
     ├─ ship: delivery mode ► PR/local merge ► teardown
     │
...
Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, delivery modes, optional X mode, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/architecture.md](docs/architecture.md) - maintainer architecture for the crew, supervision, worktrees, secondmates, and delivery modes.
