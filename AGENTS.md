...
- `project-local-env` - load before declaring a project-local credential or configuration absent when work runs from an isolated copy, or before Firstmate accepts that conclusion; its executable performs the presence-only lookup.
