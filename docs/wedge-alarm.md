...
When injection cannot confirm a submit past `FM_MAX_DEFER_SECS`, `inject_wedge_alarm` records the wedge and raises a bounded active alert so the stall never stays invisible.
...
Before a send attempt has a durable identity, active alerts and the tmux flash are rate-limited to at most once per max-defer window.
Once an exact escalation identity is reserved or offered, that unresolved identity can emit them only once, including across daemon recovery.
The durable marker continues to refresh so return catch-up retains the current buffered evidence.
...
`tests/fm-daemon.test.sh` covers directive parsing, pre-send rate limiting, exact-identity alert bounding, timeout and process-group cleanup, argv-safe dispatch, channel fallback, and safe `command:` summary delivery.
