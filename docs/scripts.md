...
| `fm-path-identity-lib.sh` | Shared device+inode (case-normalized-path fallback) filesystem-identity comparison for worktree and primary-checkout safety checks |
...
| `fm-project-local-env.sh` | Presence-only lookup across task-boundary project-local configuration sources           |
