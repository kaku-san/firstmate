...
`bin/fm-project-mode.sh` remains the one posture parser for the mechanical registry consumers that have no task in hand: fleet sync's `local-only` skip and home seeding's refusal and no-mistakes initialization.
Each registry entry's dated vision-anchor block is outside posture parsing; its format, the posture parser's ignore boundary, and `bin/fm-session-start.sh`'s architecture-map age cue are owned by `bin/fm-project-mode.sh`'s header.
